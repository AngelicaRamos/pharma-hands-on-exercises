...
-   filter(.data = adsl, EFFFL == "Y") ## my answer
-   filter(.data = adsl, "EFFFL" == "Y")
-   filter(.data = adsl, EFFFL = "y")
...
adsl_mrg_cnt <- adsl_mrg_cnt %>% mutate(perc = round(n/N*100, digits=1)) %>%
  left_join(Big_N_cnt) %>%
  mutate(perc_char = format(perc, nsmall=1)) %>%
  mutate(npct = paste(n, "(", perc_char, ")" ))%>%
  select(TRT01A, SEX, npct) %>%
...
adsl_merged_counts <- Big_N %>% 
                  left_join(small_n, by=c("TRT01AN", "TRT01A", "AGEGR1N", "AGEGR1")) %>% 
                  mutate(perc = round((n/N)*100, 1)) %>%
                  mutate(perc_chr = format(perc, nsmall = 1)) %>%
                  mutate(npct = paste(n, paste0("(", perc_chr, ")"))) %>%
                  select(TRT01AN, TRT01A, AGEGR1, SEX, npct) %>%
                  ungroup() %>%
                  pivot_wider(names_from = TRT01A, values_from = npct)
adsl_merged_counts
```
...
adae_saf = adsl_saf |> left_join(adae, by="USUBJID") |> select("USUBJID", "AEBODSYS") |> filter(!is.na(AEBODSYS))

adae_counts = adae_saf |> group_by(AEBODSYS) |> 
    count(name="n") |>
    mutate(perc = round((n/nrow(adae_saf))*100, 1), 1) |>
    mutate(perc_char = format(perc, nsmall=1)) |>
    mutate(N_PERCENT = paste(n, paste0("(", perc_char, ")"))) |>
    select(AEBODSYS, N_PERCENT)
