...
1ST METHOD: USING lapply()

```{r}
results_by_race <- bind_rows(
  lapply(unique(adsl_saf$RACE), function(race) {
    subset_data <- filter(adsl_saf, RACE == race)
    result <- adsl_counts(subset_data)
    result$RACE <- race
    return(result)
  })
) %>%
