...
gen_counts <- function(data, ..., name = "N") {
  table_counts <- data |>
    group_by(...) |>
    count(name = name)
  return(table_counts)
...
# Function to merge BigN and smallN
merge_tables <- function(ds1, ds2, ...) {
  return(ds1 |>
           left_join(ds2, by = c(...)))
}

# Calculating count for each treatment group
BIGNcnt <- gen_counts(adsl_saf, TRT01AN, TRT01A, name = "N")

# Calculating count for each category of SEX
SEX_small_n_cnt <- gen_counts(adsl_saf, TRT01AN, TRT01A, SEX, name = "n")

# Merge tables and calculate percentages
SEX_adsl_mrg_cnt <- merge_tables(SEX_small_n_cnt, BIGNcnt, "TRT01A", "TRT01AN") |>
  mutate(
...
## Calculating count for each category of AGE
AGEGR_small_n_cnt <- gen_counts(adsl_saf, TRT01AN, TRT01A, AGEGR1, name = "n")

# Merge tables and calculate percentages
AGEGR_adsl_mrg_cnt <- merge_tables(AGEGR_small_n_cnt, BIGNcnt, "TRT01A", "TRT01AN") |>
  mutate(
    perc = round((n / N) * 100, digits = 1),
    perc_char = format(perc, nsmall = 1),
    npct = paste(n, paste0("(", perc_char, "%)"))
