...
race_pct <- race_data_new %>%
  mutate(perc_tchar = format(perc_tot, nsmall = 1)) %>%
  mutate(perc_rchar = format(perc_race, nsmall = 1))
race_pct

race_n_pct <- race_pct %>%
  mutate(npct = paste(race_n, paste0("(", perc_rchar, ")"))) %>% 
...
dm_allcomb.race <- bind_rows(race_cat, agestat_cat)

dm_allcomb.race <- dm_allcomb.race |>
      mutate(category = factor(category,
                          levels = c("N", "AMERICAN INDIAN OR ALASKA NATIVE", "BLACK OR AFRICAN AMERICAN","WHITE", "Mean", "Median", "Std Dev", "Range(min,max)"))) |>
      arrange(category)
...
#           count(name="n") |>
#   ungroup() |>
#   complete(nesting(TRT01A, TRT01AN), ETHNIC, fill = list(n=0))
# eth_cnt
```
