...
```{r}
age_stat2<-age_stat %>%
  mutate(range_minmax= paste(paste0("(", min, ","), paste0(max, ")")))
...
```{r}
varSummary <- function(data, sum_var=AGE, ...) {
  data %>%
    group_by(...) %>%
    summarize(mean = mean({{sum_var}}) %>% round(digits = 1) %>% format(nsmall=1)  ,
              sd = sd({{sum_var}}) %>% round(digits = 1) %>% format(nsmall = 1), 
              med = median({{sum_var}}) %>% format(nsmall = 1)  ,              
              min = min({{sum_var}}) %>% format(nsmall = 1), 
              max = max({{sum_var}}) %>% format(nsmall = 1),
              n = n()) |>
  mutate(Range_minMax = paste(paste0("(", min, ","), paste0(max, ")"))) |>
  mutate(across(where(is.numeric), .fns=as.character)) |>
  ungroup()
...
#varSummary(data=adsl_eff, sum_var=AGE, TRT01AN, TRT01A, SEX)
adsl_wt_stat <- adsl_eff |> select("WEIGHTBL", "TRT01AN", "TRT01A", "SEX") |>
  filter(!is.na(WEIGHTBL)) |>
  varSummary(sum_var=WEIGHTBL, TRT01AN, TRT01A, SEX) |>
  select("TRT01A", "SEX" , "n", "mean", "sd", "med", "Range_minMax") |>
  pivot_longer(-c(TRT01A, SEX),names_to = "Category", values_to = "Values") |>
  mutate(Category = case_when(Category == "n" ~ "n",
                              Category == "mean" ~ "Mean",
