...
data <- mtcars %>%
  ...
  mutate(cyl = as.factor(cyl))  # Convert to factor for better x-axis labels

# Plot
ggplot(data, aes(x = cyl, y = avg_mpg)) + 
  ...
  ylab("Average MPG") +
  geom_text(aes(label = round(avg_mpg, 1)), vjust = 1.5, colour = "white", position = position_dodge(0.9), size = 5) +
  ggtitle("Average MPG by Cylinder", subtitle = "Based on mtcars dataset")
