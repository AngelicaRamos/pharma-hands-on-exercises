...
```{r}
plot2 <- plot1 + 
  labs(title = "Heart Rates across Study Days",
...
Pulse <- ADVS |> filter(PARAMCD == "PULSE")

Pulse_plot <- plot4 %+%
  Pulse

xLab <- quo_name(Pulse_plot$mapping$x)
yLab <- quo_name(unique(Pulse$PARAM))
groupLab <- quo_name(Pulse_plot$mapping$group)

study <- uniqueVal(Pulse_plot$data$STUDYID)
measure <- uniqueVal(Pulse_plot$data$PARAM)
